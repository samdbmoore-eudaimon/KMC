# Maths Card Battle – Complete Game Rules & Implementation Specification v1.1
...
The card's **Primary Type** is stored explicitly.

For Regular and Boss Cards, determine Primary Type when the card data is created or imported:

- highest statistic wins
- if two or more highest statistics are tied, the lowest-index statistic wins
...
Gameplay should then use the stored `primaryType` rather than recalculating it at runtime.

Placeholder Cards use the separate random tied-highest rule defined in Section 7.
...
At the beginning of each round, randomly select:

## 3 distinct active lanes

Only the three active lanes are visible or usable during that round. Cards, Ingenuity Tokens and movement effects may target only those lanes.

Generate the complete three-round lane schedule at battle start. The schedule must satisfy all of the following:

- each round contains three distinct Types
- all five Types appear at least once during the battle
- no Type appears in all three rounds
- the left-to-right order of the three Types is randomised independently for each round

The three visible positions are referred to as the left, middle and right lanes. Movement may target any other active lane; inactive Types are not battlefield positions for the round.

Scores in the three active lanes begin each round at:
...
2. select one of the three active lanes
...
If the chosen lane is different from the card's stored Primary Type, its owner gains:

## 1 Ingenuity Token

The comparison uses the lane where the card is played and the card's stored Primary Type. Science abilities that substitute a different statistic do not change this comparison. Movement does not award Ingenuity because movement is not a new play.

## 9A. Ingenuity Tokens

An Ingenuity Token is a temporary +1 token-card earned by playing a card outside its Primary Type.

On a normal turn, a player may choose an Ingenuity Token instead of a card from their hand, then choose one active lane. Playing the token:

1. consumes one owned Ingenuity Token
2. places a token-card into the chosen active lane
3. adds +1 to that lane
4. counts as the player's PLAY action for the turn
5. passes priority to the opponent normally

Tokens may be selected directly by clicking the token supply, then clicking an active lane. A player may play a token whenever they could normally PLAY. They may not play one while another ability is being resolved or after they have passed.

Ingenuity Tokens persist between rounds within the same battle. Played token-cards leave the battlefield at round end. Unspent tokens disappear when the battle ends.

An Ingenuity Token has no character, BV, printed statistics, Primary Type, rarity or named Set. Therefore it:

- does not award another Ingenuity Token
- does not count towards Type or Set thresholds
- does not trigger Type, Set or other abilities requiring qualifying printed data
- cannot be moved, selected, doubled, discarded, drawn, collected or staked

It does count as a card played for turn order, passing decisions, play history and effects that explicitly count all cards played rather than cards of a particular Type or Set.
...
## 2 of the 3 active lanes is sufficient to win.

A tied lane belongs to neither player.

If both players control the same number of lanes, the round is a draw. Total score is not used as a tiebreak.
...
1. shuffle the deck
2. draw 8 cards
3. leave 7 cards in the Reserve Deck
...
## Draw 3 cards from the Reserve Deck.

After Round 2:

## Draw 3 cards from the Reserve Deck.

If fewer than three cards remain, draw all remaining cards.
...
Look at the top card of your Reserve Deck.

You may discard it.

If you do, add that card's:

## printed Arithmetic statistic

to your Arithmetic lane.

The discarded card is removed for the remainder of the battle.

It is **discarded, not played**, so:

- it does not count towards Set thresholds
- its Type or Set effects do not trigger
- no other play-triggered ability on that card activates
...
When a Geometry-type card is played, the visible lane immediately to its right receives the bonus automatically.

That lane receives:

## +1

If the Geometry card is played in the right lane, the bonus wraps to the left lane.
...
Immediately before you pass, move one of your played Geometry-type cards to:

## any lane

When moved:

1. remove its current contribution from its original lane
2. move the card to the chosen destination lane
3. recalculate its contribution using its printed statistic for the destination lane
4. resolve the movement
5. immediately pass
...
- Geometry Affinity does not re-trigger
- the moved card does not count again towards Set thresholds
- no other normal play-triggered effect re-triggers
- after Symmetry resolves, the player passes and takes no further actions that round
...
Breakthrough changes the statistic used for the contribution, but it does **not** change the card's Primary Type.

For example, if a Geometry-type card uses Breakthrough while being played into Arithmetic, Geometry Affinity can still trigger normally. Any adjacent-lane effect is based on the lane where the card was actually played.
...
The response card is otherwise treated as a normal played card.

Therefore it:

- counts towards Set thresholds for the current round
- can trigger a Set's 2-card or 3-card effect
- can trigger other normal play-based effects where their conditions are met
...
A card played through Speed Mastery: Initiative counts as a normal played card for these thresholds.
...
This includes contribution bonuses currently attached to that card, such as a Gifford movement bonus.

Other unrelated lane-level bonuses are not duplicated.

For implementation, distinguish between:

- `card.currentContribution`
- lane-level or static bonuses

Reckoning doubles `card.currentContribution` only.
...
Move one opposing card into any other active lane.

Its full current contribution is removed from its original lane.
...
Do not carry its previous `currentContribution` or temporary contribution modifiers from its old position into the new lane.
...
For implementation, distinguish clearly between:

- **CARD_PLAYED_FROM_HAND**
- **CARD_MOVED**
- **CARD_CONTRIBUTION_MODIFIED**
- **LANE_BONUS_APPLIED**

A movement effect is not a new play and therefore does not re-trigger normal "when played" abilities unless a rule explicitly says otherwise.
...
- the scores and Types of the three currently active lanes
- each player's available Ingenuity Tokens
...
DRAW_AFTER_ROUND_1 = 3
DRAW_AFTER_ROUND_2 = 3

LANE_COUNT = 5
ACTIVE_LANES_PER_ROUND = 3

INGENUITY_GAIN_FOR_OFF_TYPE_PLAY = 1
INGENUITY_TOKEN_CONTRIBUTION = 1
...
PlayedCard
    ...
    printedLaneStat
    currentContribution
    temporaryContributionModifiers
    movedThisRound
```

Lane-level bonuses should be tracked separately from card contribution.

For example:

```text
LaneState
    cardContributionTotal
    staticBonuses
    totalScore
```
...
- movement removes the card's **current contribution** from its old lane
- movement then recalculates from the printed statistic for the destination lane
- Little Reckoning: Reckoning doubles the selected card's `currentContribution`
- unrelated lane-level bonuses are not doubled
...
- passed state
- active lanes, then load the next three lanes from the battle's pre-generated schedule
...
- 8-card opening hand
- two-card Mulligan
- three cards drawn between rounds
- five possible lane Types with only three visible each round
- a battle schedule in which every Type appears at least once and no Type appears in all three rounds
- two active lanes normally sufficient to win a round
- one Ingenuity Token for playing a card outside its Primary Type
- Ingenuity Tokens act as +1 token-cards and consume a normal PLAY action
- best-of-three structure
- lane-control victory
- equal numbers of controlled lanes produce a draw with no score tiebreak
- passing
